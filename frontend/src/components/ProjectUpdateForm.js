...
class ProjectUpdateForm extends React.Component{
    constructor(props){
        ...
        console.log(id)
        let project = this.props.all_projects.filter((project) => project.id == parseInt(id))
        this.state = {
            name: project[0]["name"],
            link: project[0]["link"],
    ...
    handleSubmit(event){
        this.props.updateProject(this.props.match.params.id, this.state.name, this.state.link, this.state.users)
